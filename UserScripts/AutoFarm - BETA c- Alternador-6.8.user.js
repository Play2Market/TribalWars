// ==UserScript==
// @name         AutoFarm | BETA c/ Alternador
...
(function () {
  ...
  const intervaloMin = 5 * 60000;   // 5 minutos
  const intervaloMax = 15 * 60000;  // 15 minutos
  const maxMuralha = 0;
  ...
  async function executarAtaques() {
    ...
    if (indiceAtual === -1) {
      ...
      const novaURL = atualizarParametroURL('village', aldeiaAtual);
      log(`Aldeia atual inválida. Indo para aldeia ID: ${aldeiaAtual}`);
      await sleep(3000);
    ...
    if (ataquesNestaAldeia > 0) {
      log(`Ataques enviados: ${ataquesNestaAldeia}. Esperando ~${Math.round(delayReload / 60000)} min antes de recarregar.`);
      await sleep(delayReload);
      location.reload();
      return;
    } else {
      if (aldeiasDisponiveis.length > 1) {
        const proximaIndex = (indiceAtual + 1) % aldeiasDisponiveis.length;
        const proximaAldeia = aldeiasDisponiveis[proximaIndex];
        if (proximaAldeia !== aldeiaAtual) {
          log(`Nenhum ataque possível na aldeia atual. Alternando para aldeia ID: ${proximaAldeia} após ${Math.round(delayReload / 60000)} min.`);
          await sleep(delayReload);
          const novaURL = atualizarParametroURL('village', proximaAldeia);
          location.href = novaURL;
          return;
        }
      }
      // Só uma aldeia ou voltou para mesma aldeia sem ataque
      log(`Nenhum ataque possível na aldeia. Esperando ~${Math.round(delayReload / 60000)} min antes de tentar novamente.`);
      await sleep(delayReload);
      location.reload();
    }
  }
  ...
  function createConsoleModal() {
    ...
    icon.style.cssText = `
      ...
      background-color: #E9D0A9; margin: 10px; background-position: center;
      background-repeat: no-repeat; cursor: pointer; position: relative;
      text-align: center; box-shadow: rgba(60, 30, 0, 0.7) 2px 2px 2px;
      border-radius: 3px;
    ...
    modal.style.cssText = `
      display: none; position: fixed; top: 50%; left: 50%; width: 450px;
      max-height: 300px; transform: translate(-50%, -50%); background: #2e1f00;
      border: 2px solid #603000; border-radius: 6px; box-shadow: 0 0 10px #603000;
      color: #fff; font-family: monospace; font-size: 12px; overflow-y: auto;
    ...
    btnClose.style.cssText = `
      ...
      border: none; color: #fff; cursor: pointer; padding: 2px 8px;
    ...
    toggleBtn.style.cssText = `
      margin-top: 5px; background: #785000; border: none; color: #fff;
